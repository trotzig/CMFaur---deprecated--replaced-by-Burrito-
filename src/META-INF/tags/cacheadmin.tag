<%@tag import="cmfaur.util.FriendlyDateFormatter"%>
...
<div class="cache-admin-controls">
	<ul>
		<li>
			<cmfaur:message key="cache_admin_header" extra="<%= expString %>"/>
		</li>
		...
		<li>
			<a href="/admin/clearcache?key=${key}" class="cache-admin-controls-clearcache" id="clearcache-${key}" title="${key}">
				<c:set var="desc"><cmfaur:message key="cache_admin_clear_cache_desc"/></c:set>
				<span title="${desc}"><cmfaur:message key="cache_admin_clear_cache"/></span>
			</a>
		</li>
